...
- Longer jobs, more resources, or GPUs: Run Snakemake with Slurm support
  enabled.
...
.. tip::
    ...
    ``snakemake-executor-plugin-slurm`` plugin. For more information,
    see the `Snakemake slurm plugin`_ page.
...
- ``--latency-wait 60``: This option increases the length of time
  ...
  different node than snakemake itself and since it may take some amount
  of times for files to propagate over the network file system.
- ``--rerun-incomplete``: This option ensures that snakemake reruns jobs
  that were not run to completion.
