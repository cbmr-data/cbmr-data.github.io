...
*******************************************************************
 Transferring data between Esrum and another PC, laptop, or server
*******************************************************************

If you wish to transfer data from another PC, laptop, or server (system)
to Esrum, or vice versa, then you have two options:

1. From Esrum, you can upload data to the other system or download data
   to Esrum. This requires that you can connect to the other system from
   Esrum. This option is preferable if it is expensive or otherwise
   difficult to run transfers on the other system.
2. From the other system, you can upload data to Esrum or download data
   from Esrum. This requires that you can run transfers on that system,
...
.. warning::
    ...
    Do not attempt to transfer data using ``esrumhead01fl`` as the
    source or destination; small transfers to/from your home folder are
    permitted, but your home folder is also not intended to contain
    data. Larger transfers involving ``esrumhead01fl`` will be
    terminated without warning, due to the impact on other users
...
Uploading/downloading data is done as in the :ref:`s_rsync_basics`
section, except that you have to use 2-factor authentication when
connecting to ``sftp.ku.dk``, depending on how you have configured `UCPH
...
.. warning::
    ...
    Not all software can be used to connect to ``sftp.ku.dk``, due to
    the use of two-factor authentication. For example, ``lftp`` is known
    to fail to log in because of this.
...
1. The Computerome administrators will terminate any attempts at
...
2. While it is possible to run a transfer on Computerome by running
